...
export function drawCircles() {
  ...
  if (rond4Circle && rond4Circle.image && rond4Circle.image.complete) {
    ...
    if (winCount < 2) {
      // Niveau 1 - Effet spécial pour rond4
      ...
      gradient.addColorStop(0, "#FF69B4"); // Rose foncé (comme images[0])
      gradient.addColorStop(0.5, "#FFB6C1"); // Rose moyen (comme images[1])
      gradient.addColorStop(0.8, "#FF69B4"); // Rose foncé
      gradient.addColorStop(1, "#C71585"); // Rose très foncé pour le contour

      // Dessiner l'ombre intérieure
      ctx.beginPath();
      ctx.arc(rond4Circle.x, rond4Circle.y, rond4Circle.radius, 0, Math.PI * 2);
      ctx.fillStyle = gradient;
      ctx.fill();

      // Ajouter un contour intérieur dégradé
      const innerGradient = ctx.createRadialGradient(
        rond4Circle.x,
        rond4Circle.y,
        rond4Circle.radius * 0.8,
        rond4Circle.x,
        rond4Circle.y,
        rond4Circle.radius
      );
      innerGradient.addColorStop(0, "rgba(255, 105, 180, 0)"); // Transparent
      innerGradient.addColorStop(1, "rgba(199, 21, 133, 0.7)"); // Rose très foncé semi-transparent
      ...
      // Niveau 2 - Effet spécial pour rond44 avec dégradé de verts
      const gradient = ctx.createRadialGradient(
        rond4Circle.x,
        rond4Circle.y,
        0,
      ...
      gradient.addColorStop(0, "#2E8B57"); // Vert foncé (comme images[0])
      gradient.addColorStop(0.5, "#3CB371"); // Vert moyen (comme images[1])
      gradient.addColorStop(0.8, "#2E8B57"); // Vert foncé
      gradient.addColorStop(1, "#006400"); // Vert très foncé pour le contour
      ...
      innerGradient.addColorStop(0, "rgba(46, 139, 87, 0)"); // Transparent
      innerGradient.addColorStop(1, "rgba(0, 100, 0, 0.7)"); // Vert très foncé semi-transparent
  ...
  for (let circle of circles) {
    if (circle.image) {
      ...
      // Déterminer la couleur en fonction de l'index de l'image et du niveau
      let color;
      const imageIndex = images.indexOf(circle.image);

      if (winCount >= 2) {
        // Niveau 2 - Dégradé de verts
        if (imageIndex === 0) {
          ...
          color = "white"; // Blanc pour le dernier groupe
        }
      } else {
        // Niveau 1 - Dégradé de roses
        if (imageIndex === 0) {
          color = "#FF69B4"; // Rose foncé
        } else if (imageIndex === 1) {
          color = "#FF1493"; // Rose plus foncé
        } else if (imageIndex === 2) {
          color = "#DB7093"; // Rose moyen-foncé
        } else {
          color = "white"; // Blanc
        }
      }

      // Dessiner le cercle avec la couleur appropriée
      ctx.beginPath();
